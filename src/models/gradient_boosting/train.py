"""GradientBoosting model: train and log to MLflow or run_dvc for DVC pipeline."""
import pathlib
import joblib
...
def run(X_train, X_test, y_train, y_test, feature_names, args, root: pathlib.Path):
    ...
    with mlflow.start_run():
        ...
        plot_feature_importance(model, feature_names, importance_path, "GradientBoosting", top_n=25)
        mlflow.log_artifact(str(importance_path))
        importance_path.unlink(missing_ok=True)

        cm_path = root / "confusion_matrix.png"
        plot_confusion_matrix(y_test, y_pred_test, cm_path, "GradientBoosting")
        ...
        print(f"[GradientBoosting] train_accuracy={train_accuracy:.4f}, test_accuracy={test_accuracy:.4f}")
