"""CNN model (SimpleCNN): train on image + metadata, log to MLflow."""
...
def run(args, root, X_train, X_test, y_train, y_test, le_dx):
    if not TORCH_AVAILABLE:
        raise RuntimeError("PyTorch required for CNN. Install: pip install torch torchvision")
    ...
    with mlflow.start_run():
        ...
        plot_confusion_matrix(y_test, y_pred_test, cm_path, "CNN")
        ...
        print(f"[CNN] train_accuracy={train_accuracy:.4f}, test_accuracy={test_accuracy:.4f}")
...
def run_dvc(X_train, X_test, y_train, y_test, model_dir: pathlib.Path, le_dx, **kwargs):
    ...
    if not TORCH_AVAILABLE:
        raise RuntimeError("PyTorch required for CNN. pip install torch torchvision")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    n_classes = len(le_dx.classes_)
